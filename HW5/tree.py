...
class Tree(object):
    ...
    def collapse_unary(self, join_char=CU_JOIN_CHAR):
        ...
        * For each head immediately below the root:
        ...
        for head in self:
            my_is_terminal = Tree.terminal(head)
            my_is_unary = Tree.unary(head)
            if my_is_terminal:
                continue
            head.collapse_unary()
            if not my_is_unary:
                continue
            for daughter in head:
                is_daughter_terminal = Tree.terminal(daughter)
                is_daughter_unary = Tree.unary(daughter)
                if is_daughter_terminal:
                    continue
                if is_daughter_unary:
                    continue
                for granddaughter in daughter:
                    is_granddaughter_terminal = Tree.terminal(granddaughter)
                    is_granddaughter_unary = Tree.unary(granddaughter)
                    if is_granddaughter_terminal and is_granddaughter_unary:
                        continue
                head.label = head.label + join_char + daughter.label
                head.daughters = daughter[0:]
        return self
    ...
    def chomsky_normal_form(self, markovize_char=MARKOVIZE_CHAR,
        ...
        #raise NotImplementedError
        for head in self:
            if not isinstance(head, Tree):
                continue
            head.chomsky_normal_form()
            copy_of_daughters = head.daughters[:]
            i = 1
            while len(copy_of_daughters)>2:
            #for i in range(1, len(copy_of_daughters)):
                test1 = head.daughters[-i]
                test2 = head.daughters[-(i+1)]
                #test1 = head.daughters.pop()
                #test2 = head.daughters.pop()
                test3 = copy_of_daughters.pop()
                test4 = copy_of_daughters.pop()
                new_label = head.label + markovize_char + left_delimiter + test2.label + join_char + test1.label + right_delimiter
                new_tree = Tree(new_label, [])
                new_tree.append(test4)
                new_tree.append(test3)
                copy_of_daughters.append(new_tree)
                i+=1
                #head.daughters.append(new_tree)
            #head.daughters = []
            head.daughters[0:] = copy_of_daughters[:]
            head.chomsky_normal_form()
            #print(head)
        return self
    ...
    def productions(self):
        ...
        prods = []
        for head in self:
            if Tree.terminal(head):
                prods.append(head)
                all_prods.append([self.label, prods])
                continue
            prods.append(head.label)
            
            if [self.label, prods] not in all_prods:
                all_prods.append([self.label, prods])
            all_prods.extend(head.productions())
        return all_prods
