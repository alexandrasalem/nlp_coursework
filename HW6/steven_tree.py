...
class Tree(object):
    ...
    def __init__(self, label, daughters=[]):
        self.label = label
    ...
    def collapse_unary(self, join_char=CU_JOIN_CHAR):
        ...
        * For each head immediately below the root:
        ...
        for i in xrange(len(new_daughters)):
            # is it terminal? If so, base case
            if Tree.terminal(new_daughters[i]):
                continue
            # apply recursively
            new_daughters[i] = new_daughters[i].collapse_unary(join_char)
            # is head a non-unary production?
            if not Tree.unary(new_daughters[i]):
                continue
            # extract daughter of head
            daughter = new_daughters[i][0]
            # is daughther a POS tag?
            if Tree.terminal(daughter):
                continue
            # does the head immediately dominate a POS tag?
